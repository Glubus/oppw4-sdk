...
SDK core currently resolves plugin load order from `[dependencies].plugins` and rejects duplicate Lua module names at runtime. A plugin must declare each Lua module in `[lua].modules`; having only `lua.module` is not enough. Module names are normalized to lowercase and may contain only ASCII letters, digits, `_`, `-`, and `.`. Capability checks are enforced for:
