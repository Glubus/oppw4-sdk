...
pub(super) fn install(lua: &Lua) -> mlua::Result<()> {
    ...
    register_std_module(lua, "difficulty", module)
}
...
fn for_mission(lua: &Lua, query: Value) -> mlua::Result<Value> {
    ...
    let Some(difficulties) = &mission.difficulties else {
        return Ok(Value::Nil);
    };
    ...
    table.set(
        "observations",
        json::values_to_lua_array(lua, &difficulties.observations)?,
    )?;
...
fn missions(lua: &Lua, (): ()) -> mlua::Result<Table> {
    ...
    for (index, mission) in
        mission_data::missions_with(|mission| mission.difficulties.is_some()).enumerate()
    {
        table.set(
            index + 1,
            mission_data::mission_summary_table(lua, mission)?,
...
mod tests {
    ...
    #[test]
    fn std_difficulty_is_requireable() {
        let lua = Lua::new();
        ...
        let count: i64 = lua
            .load(
                ...
                local rows = difficulty.missions()
                return #rows
                "#,
            )
            .eval()
            .expect("difficulty rows");
        assert!(count >= 0);
    }
}
