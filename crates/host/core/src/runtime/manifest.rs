...
pub(crate) struct PluginManifest {
    ...
    pub(crate) registry_modules: Vec<String>,
...
fn format_manifest_error(error: PluginManifestError) -> String {
    match error {
        ...
        PluginManifestError::MissingModTable => "missing [mod] table".to_string(),
        PluginManifestError::MissingId => "missing plugin.id".to_string(),
        PluginManifestError::MissingName => "missing mod.name".to_string(),
        ...
        PluginManifestError::InvalidModEntry(entry) => {
            format!("mod entry must be a safe relative file path: {entry}")
        }
        PluginManifestError::InvalidRegistryModule(module) => {
            format!("registry module name is invalid: {module}")
        }
