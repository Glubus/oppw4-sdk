...
use crate::{
    HostApi, HostRdbPatchReadFn, PluginContext, PluginResult, VirtualFileProvider,
    CAP_CONFIG_SCHEMA, CAP_FILES_VIRTUALIZE, CAP_RDB_PATCH, CAP_REGISTRY_MODULE,
};
...
pub struct RegistryModuleFeature {
    id: &'static str,
...
#[derive(Clone, Copy, Debug)]
pub struct RegistryModuleFeatureBuilder {
    id: &'static str,
    module_name: &'static str,
    module_context: *mut c_void,
    install: plugin_abi::Oppw4RegistryModuleInstallFn,
...
impl RegistryModuleFeatureBuilder {
    pub const fn context(mut self, module_context: *mut c_void) -> Self {
        self.module_context = module_context;
        self
    }

    pub const fn build(self) -> RegistryModuleFeature {
        RegistryModuleFeature {
            id: self.id,
...
impl PluginFeature for RegistryModuleFeature {
    fn id(&self) -> &'static str {
        self.id
    }
    ...
    fn install(&self, registrar: &mut PluginRegistrar<'_>) -> PluginResult<()> {
        registrar.host().registry().register_module_descriptor(
            registrar.plugin_id(),
            self.module_name,
            self.module_context,
            self.install,
...
mod tests {
    ...
    unsafe extern "system" fn module_install(
    ...
    fn macro_helpers_construct_features() {
        let module = crate::registry_module! {
            plugin = "macro_plugin",
            module = "macro_plugin",
            install = module_install,
        };
        assert_eq!(module.module_name(), "macro_plugin");
