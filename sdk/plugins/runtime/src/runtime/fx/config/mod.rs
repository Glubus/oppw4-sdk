...
pub(crate) use plugin::{InstallMode, PluginConfig, TriggerMode};
...
pub(crate) fn register_schema(host: HostApi<'_>) {
    if let Err(error) = host
        .configs()
        .register_schema("sdk_runtime", "fx.toml", DEFAULT_CONFIG)
    {
        crate::runtime::fx::log::write_line(format!(
            "sdk.runtime.fx config schema register failed: {error}"
        ));
    }
}
...
mod tests {
    ...
    fn creates_default_config_when_missing() {
        ...
        assert!(text.contains("type = \"sdk.runtime.fx\""));
        assert_eq!(
