...
use plugin_sdk::{
    export_plugin, Oppw4ActiveCharacter, Oppw4GameStatus, Plugin, PluginContext, PluginResult,
};
...
impl Plugin for SdkRuntime {
    ...
    fn init(context: PluginContext<'_>) -> PluginResult<()> {
        ...
        unsafe {
            game.register_status_provider(ptr::null_mut(), read_game_status)?;
            game.register_active_character_provider(ptr::null_mut(), read_active_character)?;
        }
        start_active_character_probe();
...
unsafe extern "system" fn read_game_status(
...
unsafe extern "system" fn read_active_character(
    _provider_context: *mut c_void,
...
fn start_active_character_probe() {
