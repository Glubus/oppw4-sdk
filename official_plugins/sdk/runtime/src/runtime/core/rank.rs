...
impl RankValue {
    ...
    /// Returns the known raw slot for ranks that map to vanilla slots.
    pub(crate) const fn slot(self) -> Option<u8> {
        match self {
            Self::D => Some(0),
            Self::C => Some(1),
            Self::B => Some(2),
    ...
    /// Stable lowercase name intended for config and script adapters.
...
/// Mutation requested by rank handlers.
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum RankMutation {
    SetCap {
        rank: RankValue,
        enabled: bool,
    },
...
mod tests {
    ...
    fn slot_five_is_s_plus() {
        ...
        assert_eq!(RankValue::SPlus.key(), "s_plus");
    ...
    fn unknown_slot_is_preserved() {
        ...
        assert_eq!(RankValue::from_slot(9).slot(), None);
    }
